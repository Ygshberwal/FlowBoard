...
export default function HabitSidebar({ habits, streaks, logs, onAddClick }: Props) {
  ...
  return (
    <div className="w-[220px] flex-shrink-0 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 flex flex-col h-full">
      <div className="px-4 py-4 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
        <span className="text-xs font-semibold text-slate-400 dark:text-slate-300 uppercase tracking-wider">Habits</span>
        <button
          onClick={onAddClick}
      ...
      <div className="flex-1 overflow-y-auto divide-y divide-slate-50 dark:divide-slate-700">
        {habits.length === 0 ? (
          ...
          habits.map((habit) => {
            ...
            return (
              <div key={habit.id} className="px-4 py-3 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                <div className="flex items-center gap-2">
                  <div
                    className="w-3 h-3 rounded-full flex-shrink-0"
                    style={{ backgroundColor: habit.color }}
                  />
                  <span className="text-sm font-medium text-slate-700 dark:text-slate-200 truncate">{habit.name}</span>
                </div>
